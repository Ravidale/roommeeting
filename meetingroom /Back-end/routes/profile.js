...
// })
    ...
    router.get("/:id", (req, res) => {
        ...
        Profile.findById(req.params.id, (error, result) => {
            if(error){
    ...
    router.delete("/:id", (req, res) => {
        ...
        Profile.findByIdAndRemove(req.params.id, (error, result) => {
            if(error){
                res.status(500).json(error);
            }
            else{
    ...
    router.put("/", (req, res) => {
        ...
        Profile.findByIdAndUpdate(req.body._id, newObj,  (error, result) => {
            if(error){
                res.status(500).json(error);
            }
            else {
